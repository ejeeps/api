...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Check if ID number already exists (only if id_number column exists)
        try {
            // First check if the column exists by attempting to query it
            ...
            if (strpos($e->getMessage(), "Unknown column 'id_number'") !== false) {
                throw new Exception("Database migration required. Please run migration 010_add_id_number_to_passengers.sql to add the id_number column to the passengers table.");
            }
